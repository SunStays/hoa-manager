    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Community decisions, one unit one vote</p>
        </div>
        {isBoard && (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
          ...
          <p className="text-gray-500 text-sm mb-4">No votes yet.</p>
          {isBoard && (
            <button onClick={() => setShowCreate(true)} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <div className="space-y-6">
          {open.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Open</h2>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold text-gray-900 mb-5">New vote</h2>
            <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="space-y-2">
                  {options.map((opt, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      />
                      {options.length > 2 && (
                        <button type="button" onClick={() => setOptions(options.filter((_, j) => j !== i))} className="text-red-400 hover:text-red-600 px-2">✕</button>
                      )}
                    </div>
                  ))}
                </div>
                {options.length < 6 && (
                  <button type="button" onClick={() => setOptions([...options, ""])} className="mt-2 text-sm text-blue-600 hover:underline">
                    + Add option
                  </button>
                )}
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setShowCreate(false)} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50">Cancel</button>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <h2 className="text-lg font-bold text-gray-900 mb-2">Delete this vote?</h2>
            <p className="text-sm text-gray-500 mb-6">All responses will be lost. This cannot be undone.</p>
...
function VoteCard({
  ...
  return (
    <div className={`bg-white rounded-2xl border ${open ? "border-blue-100" : "border-gray-100"} overflow-hidden`}>
      {/* Header row */}
      <button onClick={onToggle} className="w-full text-left px-5 py-4 flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${open ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>
              {open ? "Open" : "Closed"}
            </span>
            <span className="text-xs text-gray-400">{open ? `Closes ${formatDate(vote.closesAt)}` : `Closed ${formatDate(vote.closesAt)}`}</span>
          </div>
          <p className="font-semibold text-gray-900">{vote.question}</p>
          <p className="text-xs text-gray-400 mt-1">{totalVotes} vote{totalVotes !== 1 ? "s" : ""} cast</p>
        </div>
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-gray-50 pt-4 space-y-5">
              ...
              return (
                <div key={opt}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-gray-500">{count} vote{count !== 1 ? "s" : ""} · {pct}%</span>
                  </div>
          ...
          {totalVotes > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Who voted what</p>
              <div className="space-y-1">
                {vote.options.map((opt) => {
                  const respondents = vote.responses.filter((r) => r.option === opt);
                  if (respondents.length === 0) return null;
                  return (
                    <div key={opt} className="bg-gray-50 rounded-xl px-3 py-2">
                      <p className="text-xs font-semibold text-gray-700 mb-1.5">{opt}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {respondents.map((r) => (
                          <span key={r.id} className="inline-flex items-center gap-1 px-2 py-0.5 bg-white border border-gray-200 rounded-full text-xs text-gray-600">
                            {r.user.name} <span className="text-gray-400">· Unit #{r.unit.unitNumber}</span>
                          </span>
                        ))}
                      </div>
                    </div>
          ...
          {open && myPendingUnits.length > 0 && (
            <div className="border border-blue-100 rounded-xl p-4 bg-blue-50">
              <p className="text-sm font-semibold text-blue-800 mb-3">Cast your vote</p>
              {myPendingUnits.map((unit) => (
                <div key={unit.id} className="mb-3 last:mb-0">
                  <p className="text-xs text-blue-700 font-medium mb-2">Unit #{unit.unitNumber}</p>
                  <div className="flex flex-wrap gap-2">
                    {vote.options.map((opt) => (
                      <button
                        ...
                        className="px-3 py-1.5 bg-white border border-blue-300 text-blue-700 text-sm font-medium rounded-lg hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors disabled:opacity-50"
                      >
                        {opt}
                      </button>
                    ))}
          ...
          {myVotedUnits.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Your votes</p>
              <div className="flex flex-wrap gap-2">
                {myVotedUnits.map((unit) => {
                  const response = vote.responses.find((r) => r.unitId === unit.id);
                  return (
                    <span key={unit.id} className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-50 border border-green-200 rounded-full text-sm text-green-700">
              ...
              {open && myPendingUnits.length === 0 && (
                <p className="text-xs text-gray-400 mt-2">All your units have voted.</p>
              )}
            </div>
          )}
          ...
          {isBoard && (
            <div className="pt-2 border-t border-gray-100">
              <button onClick={() => onDelete(vote.id)} className="text-sm text-red-500 hover:underline">Delete vote</button>
            </div>
          )}
        </div>
      )}
